...
export function renderHtmlReport(audit: AuditResult): string {
  ...
  out.push('<table>');
  out.push(
    '<caption>Per-category counts. Deliberately not rolled into one number: a composite score would need a published formula before it could mean anything.</caption>',
  );
  out.push(
    '<thead><tr><th scope="col">Category</th><th scope="col" class="num">Passed</th><th scope="col" class="num">Checked</th><th scope="col" class="num">Not applicable</th><th scope="col" class="num">Not judged</th></tr></thead>',
  );
  out.push('<tbody>');
  for (const row of audit.categories) {
    out.push(
      `<tr><th scope="row">${esc(CATEGORY_LABEL[row.category] ?? row.category)}</th>` +
        `<td class="num">${row.passed}</td><td class="num">${row.applicable}</td>` +
  ...
  out.push('</main>');
  out.push(
    `<footer><p>Generated by ${esc(audit.tool.name)} ${esc(audit.tool.version)} at ${esc(utcStamp(audit.finishedAt))}. ` +
      'This file is self-contained: it runs no scripts and requests nothing from the network.</p></footer>',
  );
  out.push('</body>');
  out.push('</html>');
