...
- `/.well-known/agent-skills/index.json` — the Agent Skills discovery index, per the Agent Skills Discovery RFC v0.2.0 (`$schema` `https://schemas.agentskills.io/discovery/0.2.0/schema.json`). It lists one skill, `using-mattpyle-com`, served at `/.well-known/agent-skills/using-mattpyle-com/SKILL.md` as `text/markdown`. The skill is the workflow version of this file: which surface answers which question, how to negotiate markdown, how to call the A2A endpoint and the WebMCP tools. Each entry's `digest` is the SHA-256 of the artifact's bytes, generated at build and verified against the built file, so it is safe to check.
