...
- `/.well-known/ard.json` — the Agentic Resource Discovery catalogue, per the ARD specification v0.91 (agenticresourcediscovery.org). Four entries, one per agentic resource this site publishes: the A2A Agent Card, the MCP discovery document, and the two Agent Skills. Each entry names the artifact's own URL rather than restating it, so the catalogue cannot contradict the documents above. The same catalogue is advertised by `<link rel="ard">` on every page and by an `Agentmap:` line in `/robots.txt`; no DNS records are published for it.
