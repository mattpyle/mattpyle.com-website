 ...
 *
 * Every page runs modern at 320, then retro at 320, 427 and 440. Retro is a real
 ...
 * representative page.
 *
 * 320 is the only width 1.4.10 asks about. The other two bracket retro's 440px
 * wrap rule, one on each side, because that rule exists to satisfy a measured
 * font metric (the unwrapped row needs 429px) and a metric is exactly the kind of
 * premise that goes stale without anything failing:
 *
 *   427  Inside the rule, the widest width that overflows without it. Deleting or
 *        narrowing the wrap fails here. 428 would not: the overflow there is 1px,
 *        inside the tolerance below, which is why this is 427 and not 429.
 *   440  Outside the rule, the narrowest width that keeps the single row, holding
 *        11px of slack over the 429 the row needs. Anything that widens the row
 *        past 440 (a sixth nav link, a longer label, different platform metrics)
 *        fails here while 320 stays green, and the answer is to move the
 *        breakpoint rather than to relax this row.
 *
 * Measured, not assumed: rebuilt against the pre-fix retro.css, the 320 and 427
 * rows fail on all nine pages and the 440 rows pass on all nine.
 */

const RETRO_WIDTHS = [320, 427, 440] as const;
